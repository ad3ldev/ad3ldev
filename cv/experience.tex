...
%---------------------------------------------------------
  \cventry
   {Technical Lead} % Job title
    ...
    {Quebec, Canada (Remote)} % Location
    {Jan. 2026 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {cache-aside}
        \item {write behind cache}
        \item {ui library}
        \item {monorepo}
        \item {VA, 2 seniors, 4 juniors}
        \item {query builder}
        \item {db2 sanitaiztion and types}
        \item {db2 repositories and such}
        \item {DataStore}
        \item {Led 5-engineer architectural audit and analysis of legacy IBM DB2 system, diagnosing systemic anomalies, inconsistencies, and corruption in 20.2M-row denormalized dataset; designed BCNF-compliant target schema to eliminate referential integrity anomalies, projecting ~53× reduction in storage footprint.}
